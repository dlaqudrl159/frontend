...
const Category4 = ({ categoryYear, categoryRegion }) => {
  const [activeTab, setActiveTab] = useState(null);
  console.log("Category2함수부분")
  const tabs = [
    { id: "year", label: categoryYear },
    { id: "choice", label: "지번주소" },
    { id: "region", label: categoryRegion },
      ...
      case "year":
        return (
          <div style={{...styles.dropdown, ...styles.yearDropdown}}>
            <h3>Select Year</h3>
            <select>
              <option>{categoryYear}</option>
              <option>{parseInt(categoryYear) - 1}</option>
              <option>{parseInt(categoryYear) - 2}</option>
            </select>
          </div>
        );
      case "choice":
